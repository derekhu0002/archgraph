...
<GraphQueryGuideline>
For structural/type-based graph lookups, use the read-only Neo4j Cypher interface `queryNeo4jGraph` instead of reading the canonical JSON file directly. It never mutates the canonical graph; all writes still go through the mutation tools. Before servicing a query, the tool detects whether the projection is stale (sync state dirty, or recorded canonicalDigest differing from the JSON); if so, it automatically rebuilds the projection from the canonical JSON (recoverNeo4jSyncIfNeeded) and reports the attempt via the `neo4jRecovery` field on the result. The agent's Cypher itself stays read-only.
