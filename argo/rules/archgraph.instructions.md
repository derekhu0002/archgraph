...
3. 启动某个 `Business Actor` 前，先读取该元素的 `description`，把它作为该 agent 的定义（system prompt）来构造 agent，再开始工作；每次收工前，必须把本次关键进展以结构化的方式回写到该 Actor 下挂的长期记忆 View中（如果没有则创建，可以下挂多个View，也可以在View中的元素下继续展开新的View，形成一个层次化的长期记忆系统），刷新长期记忆，防止长会话遗忘。
