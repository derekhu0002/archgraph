...
test('AT-rerank-04: rerankCandidates parses the chat response and fails open', async () => {
  ...
  const configuration = { embeddingBaseUrl: 'https://example/v1', qwenKey: 'k' };
  const okTransport = { request: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: '{"order":["b","a"]}' } }] }) }) };
  assert.deepEqual(await rerankCandidates({ query: 'q', candidates, configuration, transport: okTransport, maxReturn: 2 }), ['Element:b', 'Element:a']);

  const badTransport = { request: async () => ({ ok: false }) };
  assert.equal(await rerankCandidates({ query: 'q', candidates, configuration, transport: badTransport }), null);
  const throwTransport = { request: async () => { throw new Error('boom'); } };
  assert.equal(await rerankCandidates({ query: 'q', candidates, configuration, transport: throwTransport }), null);
  // trivial / misconfigured inputs short-circuit
  assert.equal(await rerankCandidates({ query: 'q', candidates: [candidates[0]], configuration, transport: okTransport }), null);
  assert.equal(await rerankCandidates({ query: 'q', candidates, configuration: null, transport: okTransport }), null);
});
